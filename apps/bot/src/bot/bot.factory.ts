      ...
      try {
        ...
        if (ctx.text == KEYBOARD_COMMANDS.resendRequest) {
          ...
          switch (ctx.text) {
            ...
            case KEYBOARD_COMMANDS.profile:
              try {
                const user = await this.getUser(userId);
                if (!user) {
                  await ctx.send("⚠️ что-то пошло не так");
                  return;
                }

                const userGroups =
                  await this.userRepository.GetUserGroups(userId);
                ...
                this.logger.error({ err: e }, "profile command failed");
                await ctx.send("⚠️ что-то пошло не так");
              }
              return;
