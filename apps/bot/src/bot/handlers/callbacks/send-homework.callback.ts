...
export async function CallbackQuerySendHomework(
	...
	const homeworkId = Number.parseInt(stringHomeworkId);
	...
	try {
		...
		await props.ctx.send(
			format`→ Загрузите PDF файл для домашнего задания по ${link("ссылке", `${env.FRONTEND_URL}/?token=${token}`)} в течении 5 минут`
		);
	} catch (e) {
		props.bot.logger.error(
